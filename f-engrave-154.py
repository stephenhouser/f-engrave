...
"""
    ...
    Version 1.45 - Added multipass cutting for v-carving
                 - Removed "Inside Corner Angle" and "Outside Corner Angle" options

    Version 1.46 - Fixed bug which cause double cutting of v-carve pattern when multipass cutting was disabled
    ...
    Version 1.47 - Added ability to read more types of DXF files (files using BLOCKS with the INSERT command)
                 ...
                 - Added .tap to the dropdown list of file extensions in the file save dialog

    Version 1.48 - Fixed another bug in the multipass code resulting in multipass cutting when multipass cutting was disabled.
    ...
    Version 1.50 - Modified helper program (ttf2cxf_stream) and F-Engrave interaction with it to better control the line
	           segment approximation of arcs.
                 ...
                 - Fixed minor bug in the v-bit cleanup toolpath generation
    ...
    Version 1.51 - Added Plunge feed rate setting (if set to zero the normal feed rate applies)
                 ...
                 - Fixed some Python 3 incompatibilities with reading config files
    ...
    Version 1.53 - Changed space for leading carriage returns to only apply at 0,90,270 and 180 degree rotations.
                 - Added floating tool tips to the options on the main window (hover over the option labes to see the tool tip text)
    ...
    Version 1.54 - Fixed bug that resulted in errors if the path to a file contained the text of an F-Engrave setting variable
                 ...
                 - No searched current folder and home folder for image files when opening existing g-code files.
...
class Application(Frame):
    ...
    def createWidgets(self):
        ...
        #self.Label_Yscale_hover = HoverInfo(self.Label_Yscale, '"Text Height" sets the image/text height.')
        ...
        #self.Label_Zsafe_ToolTip = ToolTip(self.Label_Zsafe, text='Enough of this') 
...
################################################################################
    ...
    def Line_Arc_Fit(self,lastx,lasty,lastz,x1,y1,z1,nextx,nexty,nextz,FLAG_arc,code,R_last,x_center_last,y_center_last,FLAG_line):
        ...
        if (FLAG_line != 1 and FLAG_line_last !=1 and line_t != 0 and self.arc_fit.get()!="none"):
            dx_b = nextx-x1
            dy_b = nexty-y1
            dz_b = nextz-z1
    ...
    def WriteGCode(self,config_file=False):
        ...
        self.gcode.append('( by Scorch - 2015 )')
        ...
        #FORMAT = 'G1 Z%s F%%.%df  ( Set Feed Rate )' %(safe_val,dpfeed)
        ...
        if self.cut_type.get() == "engrave" or self.bit_shape.get() == "FLAT":
            ...
            while (rough_again == True or roughing == True):
                ...
                #self.gcode.append("( BEGINING CUT LOOP )")
                #self.gcode.append("( zmin=%f maxDZ=%f)" %(zmin,maxDZ))
                            ...
                            if ( WRITE == 1 or nextloop == -99):
                                ...
                                if (code == "G2" or code == "G3"):
                                    ...
                                    if  R_check_1 > Zero or  R_check_2 > Zero:
                                        ...
                                        self.gcode.append(FORMAT %(lastx,lasty) + FEED_STRING)
                                        self.gcode.append(FORMAT %(x1,y1))
                                        ...
                                        if (self.arc_fit.get() == "center"):
                                            ...
                                            self.gcode.append(FORMAT %(code,x1,y1,Ival,Jval) + FEED_STRING)
                                        else:
                                            ######################################################################
                                            # This is the code for the radius format for arcs
                                            FORMAT = '%%s X%%.%df Y%%.%df R%%.%df' %(dp,dp,dp)
                                            self.gcode.append(FORMAT %(code,x1,y1,R_last) + FEED_STRING)
                                            ######################################################################
                                else:
                                    FORMAT = 'G1 X%%.%df Y%%.%df' %(dp,dp) + FEED_STRING
                                    self.gcode.append(FORMAT %(x1,y1))
                                x_center_last = 0
                                y_center_last = 0
                                R_last = 0
                    ...
                    while v_index < len(new_coords)-1:
                        ...
                        if (loop != loop_old):
                            ...
                            if (feed_current == plunge_str):
                                ...
                                self.gcode.append(FORMAT %(z1) + " F" + plunge_str)
                                feed_current = plunge_str
                            ...
                            if ( WRITE == 1 or nextloop == -99):
                                ...
                                if (code == "G2" or code == "G3"):
                                    ...
                                    if  R_check_1 > Zero or  R_check_1 > Zero:
                                        ...
                                        self.gcode.append(FORMAT %(lastx,lasty,lastz)+ FEED_STRING)
                                        ...
                                        self.gcode.append(FORMAT %(code,x1,y1,Ival,Jval,z1) + FEED_STRING)
                                        ######################################################################
                                    ...
                                    FORMAT = 'G1 X%%.%df Y%%.%df Z%%.%df' %(dp,dp,dp)
                                    self.gcode.append(FORMAT %(x1,y1,z1) + FEED_STRING)
                                lastx=x1
                                lasty=y1
                                lastz=z1
                                x_center_last = 0
    ...
    def WRITE_CLEAN_UP(self,bit_type="straight"):
        ...
        if not self.var_dis.get():
            FORMAT = '#1 = %%.%df  ( Safe Z )' %(dp)
            self.gcode.append(FORMAT %(SafeZ))
            #FORMAT = '#2 = %%.%df  ( Cutting Depth Z )' %(dp)
            #self.gcode.append(FORMAT %(Depth))
            safe_val  = '#1'
            #depth_val = '#2'
        else:
        ...
        if abs(self.w-w)>10 or abs(self.h-h)>10 or update==1:
            ...
            if self.input_type.get() == "text":
                ...
                if self.cut_type.get() != "engrave":
                    ...
                    #self.Entry_Plunge.configure(state="disabled")
                    #self.Label_Plunge.configure(state="disabled")
                    #self.Label_Plunge_u.configure(state="disabled")
                else:
                    self.Entry_Zcut.configure(state="normal")
                    self.Label_Zcut.configure(state="normal")
                    self.Label_Zcut_u.configure(state="normal")
                    #self.Entry_Plunge.configure(state="normal")
                    #self.Label_Plunge.configure(state="normal")
                    #self.Label_Plunge_u.configure(state="normal")
...
##
##                if (self.v_flop.get()!=0 and (self.input_type.get() == "text") ):
##                    self.coords.append([ minx-Delta, miny-Delta, maxx+Delta, miny-Delta, 0, 0])
##                    self.coords.append([ maxx+Delta, miny-Delta, maxx+Delta, maxy+Delta, 0, 0])
##                    self.coords.append([ maxx+Delta, maxy+Delta, minx-Delta, maxy+Delta, 0, 0])
##                    self.coords.append([ minx-Delta, maxy+Delta, minx-Delta, miny-Delta, 0, 0])
        ...
        if self.cut_type.get() == "v-carve" and self.fontdex.get() == False:
            ...
            if ((self.input_type.get() == "image") and (clean_flag != 1)):
                ...
                while i < len(Lbeg): #for each loop
                    ...
                    if  dist <= Acc: #if end is the same as the beginning
            ...
            if TOT_LENGTH > 0.0:
                ...
                for line in range(len(self.coords)):
                    ...
                    if delta < float(v_drv_crner) and BIT_ANGLE !=0 and not_b_carve and clean_flag != 1:
                        ...
                        #if self.inlay.get():
                        #    inlay_angle = radians( ((theta-delta+90) + (theta+90))/2)
                        #    xv,yv,rv,clean_seg=self.record_v_carve_data(x1,y1,inlay_angle,r_inlay_top,loop_cnt,clean_flag)
                        #    self.vcoords.append([x1, y1, 0.0, loop_cnt])
                    ...
                    if (abs(x2-xa) < Acc and abs(y2-ya) < Acc):
                        ...
                        if delta < v_drv_crner and clean_flag != 1:
                            ...
                            #if self.inlay.get():
                            #    inlay_angle = radians( ((theta-delta+90) + (theta+90))/2)
                            #    xv,yv,rv,clean_seg=self.record_v_carve_data(x2,y2,inlay_angle,r_inlay_top,loop_cnt,clean_flag)
                            #    self.vcoords.append([x2, y2, 0.0, loop_cnt])
